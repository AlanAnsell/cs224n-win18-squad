...
class BDAttn(object):
    ...
    def __init__(self, keep_prob, key_vec_size, value_vec_size):
        ...
        Inputs:
          ...
          value_vec_size: size of the value vectors. int
        """
        assert key_vec_size == value_vec_size
        self.keep_prob = keep_prob
        self.key_vec_size = key_vec_size
        self.value_vec_size = value_vec_size
    ...
    def build_graph(self, values, values_mask, keys, keys_mask, q2c=True):
        ...
        Outputs:
          ...
          output: Tensor shape (batch_size, num_keys, hidden_size).
            ...
            (using the attention distribution as weights).
        """
        with vs.variable_scope("BDAttn"):
            w_keys = tf.get_variable('w_keys', shape=(1, 1, self.key_vec_size), dtype=tf.float32,
                                     initializer=tf.contrib.layers.xavier_initializer())
            w_values = tf.get_variable('w_values', shape=(1, 1, self.key_vec_size), dtype=tf.float32,
                                       initializer=tf.contrib.layers.xavier_initializer())
            w_prod = tf.get_variable('w_prod', shape=(1, 1, self.key_vec_size), dtype=tf.float32,
                                     initializer=tf.contrib.layers.xavier_initializer())
        
            # Calculate attention distribution
            values_t = tf.transpose(values, perm=[0, 2, 1]) # (batch_size, value_vec_size, num_values)
            weighted_prod = tf.matmul(w_prod * keys, values_t)
